...
export function ComunicacaoClientLayout({
    children,
}: {
    ...
    return (
        <DashboardLayout>
            <div className="w-full h-full flex-1 min-h-0 flex flex-col max-w-7xl mx-auto gap-6 px-4 sm:px-0">
                {/* Header Module */}
                <div className="shrink-0 flex flex-col md:flex-row justify-between items-end md:items-center gap-4 border-b border-slate-200 dark:border-white/10 pb-4">
                    ...
                    <div className="flex flex-wrap items-center gap-4">
                        <div className="flex gap-1 bg-slate-100 dark:bg-zinc-900 p-1 rounded-lg">
                            {TABS.map(tab => {
                                ...
                                return (
                                    <Link
                                        ...
                                        className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-all ${isActive
                                            ? 'bg-white dark:bg-zinc-800 text-primary dark:text-amber-500 shadow-sm'
                                            : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-amber-400 hover:bg-slate-200/50 dark:hover:bg-white/5'
                                            }`}
                                    >
                                        <Icon size={16} />
                                        {tab.name}
