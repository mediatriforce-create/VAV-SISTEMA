...
export default function PedagogiaPage() {
    ...
    const [stats, setStats] = useState({ classes: 0, activities: 0, kanban: 0 })
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            const [profileRes, classesRes, activitiesRes, kanbanRes] = await Promise.all([
                ...
                supabase.from('ped_activities').select('id', { count: 'exact', head: true })
                    .gte('activity_date', new Date().toISOString().split('T')[0]),
                supabase.from('ped_kanban_cards').select('id', { count: 'exact', head: true })
                    .in('column_status', ['planejado', 'andamento', 'aprovacao']),
            ])
                    ...
                    <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            ...
                            <h2 className="text-2xl sm:text-3xl font-black text-zinc-900 dark:text-white leading-tight">
                                Olá, <span className="text-emerald-600 dark:text-emerald-400">{firstName}</span> 👋
                            </h2>
                        ...
                        <div className="flex gap-3 shrink-0">
                            {[
                                { label: 'Turmas', value: stats.classes, icon: 'groups' },
                                { label: 'Hoje', value: stats.activities, icon: 'today' },
                                { label: 'Em curso', value: stats.kanban, icon: 'pending_actions' },
                            ].map((s) => (
