...
export default function AtividadesPage() {
    ...
    const loadDriveFolders = async () => {
        ...
        try {
            ...
            ['1 ANO', '2 ANO', '3 ANO', '4 ANO', '5 ANO'].forEach(name => {
                if (commFolderIds[name]) allFolders.push({ id: commFolderIds[name], name, source: 'comm' });
            });
    ...
    const handleCreate = async () => {
        ...
        if (res.success && res.data) setActivities(prev => [res.data!, ...prev]);
            ...
            <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar space-y-4 pr-1">
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                        ...
                        <p className="text-xs text-zinc-400 mt-1">Selecione outra data ou clique em "Nova Atividade".</p>
                    </div>
                ) : (
                    activities.map(act => (
                        <motion.div key={act.id}
                            ...
                            <div className="flex items-start justify-between mb-2">
                                ...
                                <span className="text-[10px] font-medium text-zinc-400">{new Date(act.activity_date + 'T12:00:00').toLocaleDateString('pt-BR')}</span>
                            </div>
            ...
            <AnimatePresence>
                {showModal && (
                    ...
                    >
                        ...
                        >
                            {/* Header */}
                            <div className="shrink-0 p-5 border-b border-zinc-100 dark:border-zinc-800">
                            ...
                            <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-5 space-y-4">
                                ...
                                <div>
                                    ...
                                    <input value={formTitle} onChange={e => setFormTitle(e.target.value)} placeholder="Ex: Jogo de Matemática com tampinhas"
                                        className="w-full px-4 py-2.5 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 rounded-xl outline-none focus:border-emerald-500 text-sm" />
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <textarea value={formDesc} onChange={e => setFormDesc(e.target.value)} rows={2} placeholder="O que foi feito..."
                                            className="w-full p-3 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 rounded-xl outline-none text-sm resize-none" />
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-zinc-500 uppercase tracking-wider mb-1 block">Observações</label>
                                        <textarea value={formNotes} onChange={e => setFormNotes(e.target.value)} rows={2} placeholder="Notas extras..."
                                            className="w-full p-3 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 rounded-xl outline-none text-sm resize-none" />
                                    </div>
                                </div>

                                {/* === FOTOS DO DIA === */}
                                <div className="border border-emerald-200 dark:border-emerald-800 rounded-xl overflow-hidden bg-emerald-50/30 dark:bg-emerald-500/5">
                                    <div className="px-4 py-2.5 border-b border-emerald-200 dark:border-emerald-800 flex items-center gap-2">
                                        ...
                                        <span className="text-xs font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider">Fotos do Dia</span>
                                    </div>
                                    <div className="p-4 space-y-3">
                                        ...
                                        <div>
                                            ...
                                            {foldersLoading ? (
                                                ...
                                                <select value={selectedFolderId} onChange={e => setSelectedFolderId(e.target.value)}
                                                    className="w-full px-3 py-2 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl outline-none text-sm focus:border-emerald-500">
                                        ...
                                        {/* Botão de upload + lista */}
                                        {selectedFolderId && (
                                            <>
                                                <input ref={fileInputRef} type="file" multiple accept="image/*" className="hidden" onChange={handleFilesSelected} />
                                                <button type="button" onClick={() => fileInputRef.current?.click()}
                                                    className="w-full border-2 border-dashed border-emerald-300 dark:border-emerald-700 rounded-xl py-3 flex items-center justify-center gap-2 text-emerald-500 hover:text-emerald-600 hover:border-emerald-400 transition-colors text-xs font-bold">
                                                ...
                                                {selectedFiles.length > 0 && (
                                                    <div className="space-y-1">
                                                        <p className="text-[10px] font-bold text-zinc-500">{selectedFiles.length} foto(s)</p>
                                                        {selectedFiles.map((file, i) => (
                                                            <div key={i} className="flex items-center gap-2 bg-white dark:bg-zinc-800 rounded-lg px-3 py-1.5 text-xs">
                                                                ...
                                                                <button onClick={() => removeFile(i)} className="text-zinc-300 hover:text-red-500">
                            ...
                            {/* Footer */}
                            <div className="shrink-0 p-5 border-t border-zinc-100 dark:border-zinc-800 flex items-center gap-3">
                                ...
                                <div className="flex items-center gap-3 ml-auto">
                                    ...
                                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleCreate} disabled={submitting || !formTitle.trim()}
                                        className="bg-gradient-to-r from-emerald-500 to-teal-600 text-white px-5 py-2 rounded-xl text-sm font-bold shadow-lg shadow-emerald-500/25 disabled:opacity-50">
                                        {submitting
                                            ? (selectedFiles.length > 0 ? 'Enviando...' : 'Salvando...')
                                            : (selectedFiles.length > 0 ? `Registrar + ${selectedFiles.length} foto(s)` : 'Registrar')
                                        }
                                    </motion.button>
                                </div>
                            </div>
